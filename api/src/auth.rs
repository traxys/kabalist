...
use openidconnect::{
    AccessToken, AuthorizationCode, CsrfToken, Nonce, OAuth2TokenResponse, PkceCodeChallenge,
    PkceCodeVerifier, ProviderMetadata, RedirectUrl, Scope, UserInfoClaims,
};
use sqlx::PgPool;
use tracing::*;
use uuid::Uuid;
...
pub struct Oauth2Client {
    ...
    key: Key,
    users: Arc<std::sync::Mutex<HashMap<String, Uuid>>>,
}
...
impl<S: Send + Sync> FromRequestParts<S> for User {
    ...
    fn from_request_parts(
        ...
        async move {
            ...
            let uuid = Uuid::parse_str(&uuid_raw).map_err(|_| ())?;
            Ok(User { id: uuid })
        }
    }
}
