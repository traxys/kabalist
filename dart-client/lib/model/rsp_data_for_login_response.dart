...
class RspDataForLoginResponse {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RspDataForLoginResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RspDataForLoginResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<RspDataForLoginResponse>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RspDataForLoginResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RspDataForLoginResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RspDataForLoginResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = RspDataForLoginResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<RspDataForLoginResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RspDataForLoginResponse>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RspDataForLoginResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
