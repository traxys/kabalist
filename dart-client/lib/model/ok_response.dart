...
class OkResponse {
  ...
  /// Returns a new [OkResponse] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OkResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OkResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OkResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OkResponse> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OkResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OkResponse> mapFromJson(dynamic json) {
    final map = <String, OkResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OkResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
