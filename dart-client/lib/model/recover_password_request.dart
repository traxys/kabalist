//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.0

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

part of openapi.api;

class RecoverPasswordRequest {
  /// Returns a new [RecoverPasswordRequest] instance.
  RecoverPasswordRequest({
    @required this.password,
  });

  String password;

  @override
  bool operator ==(Object other) => identical(this, other) || other is RecoverPasswordRequest &&
     other.password == password;

  @override
  int get hashCode =>
  // ignore: unnecessary_parenthesis
    (password == null ? 0 : password.hashCode);

  @override
  String toString() => 'RecoverPasswordRequest[password=$password]';

  Map<String, dynamic> toJson() {
    final json = <String, dynamic>{};
      json[r'password'] = password;
    return json;
  }

  /// Returns a new [RecoverPasswordRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static RecoverPasswordRequest fromJson(dynamic value) {
    if (value is Map) {
      final json = value.cast<String, dynamic>();
      return RecoverPasswordRequest(
        password: mapValueOfType<String>(json, r'password'),
      );
    }
    return null;
  }

  static List<RecoverPasswordRequest> listFromJson(dynamic json, {bool emptyIsNull, bool growable,}) =>
    json is List && json.isNotEmpty
      ? json.map(RecoverPasswordRequest.fromJson).toList(growable: true == growable)
      : true == emptyIsNull ? null : <RecoverPasswordRequest>[];

  static Map<String, RecoverPasswordRequest> mapFromJson(dynamic json) {
    final map = <String, RecoverPasswordRequest>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) => map[key] = RecoverPasswordRequest.fromJson(value));
    }
    return map;
  }

  // maps a json object with a list of RecoverPasswordRequest-objects as value to a dart map
  static Map<String, List<RecoverPasswordRequest>> mapListFromJson(dynamic json, {bool emptyIsNull, bool growable,}) {
    final map = <String, List<RecoverPasswordRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json
        .cast<String, dynamic>()
        .forEach((key, dynamic value) {
          map[key] = RecoverPasswordRequest.listFromJson(
            value,
            emptyIsNull: emptyIsNull,
            growable: growable,
          );
        });
    }
    return map;
  }
}

