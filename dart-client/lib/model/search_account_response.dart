...
class SearchAccountResponse {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SearchAccountResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SearchAccountResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SearchAccountResponse> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SearchAccountResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SearchAccountResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SearchAccountResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = SearchAccountResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<SearchAccountResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SearchAccountResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SearchAccountResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
