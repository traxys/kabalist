    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GetAccountNameResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GetAccountNameResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<GetAccountNameResponse> listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GetAccountNameResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GetAccountNameResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = GetAccountNameResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
