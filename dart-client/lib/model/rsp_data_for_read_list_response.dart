...
class RspDataForReadListResponse {
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RspDataForReadListResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RspDataForReadListResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<RspDataForReadListResponse>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RspDataForReadListResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RspDataForReadListResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RspDataForReadListResponse> mapFromJson(dynamic json) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        final value = RspDataForReadListResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<RspDataForReadListResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RspDataForReadListResponse>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RspDataForReadListResponse.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
